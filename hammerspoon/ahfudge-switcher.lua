---
--- Copy Pasta from hs.window.switcher with one important difference:
--- modsPressed function can be overridden to allow for 'modifiers' other than alt/shift/etc....  like 'a' and 'f' from 'a'h 'f'udge mode
---
...
function modsPressed() return checkMods(true)._raw>0 end
local function show(self,dir)
  ...
  if not selected then -- fresh invocation, prep everything
    ...
    self.modsTimer=timer.waitWhile(self.modsPressed,function()exit(self)end,MODS_INTERVAL)
    selected=1
